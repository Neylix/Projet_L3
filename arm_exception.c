...
void arm_exception(arm_core p, unsigned char exception) {
    ...
    if (exception == RESET) {
        ...
        arm_write_register(p, 13, 0x800);
		arm_write_usr_register(p, 15, 0);
    }
	else if (exception == UNDEFINED_INSTRUCTION)
	{
		;		//Ne rien faire, on n'�mule pas l'instruction ind�finie
	}
	else if (exception == DATA_ABORT)
	{
		;		//Ne rien faire, on ne touche pas � la m�moire d�j� allou�e et on n'en r�alloue pas plus
	}
}
